  ...
  addOrUpdateApple(appleToken, mail, callback) {
    this.pool.getConnection(function (err, connection) {
      if (err) {
        ...
        connection.query(
          "SELECT * FROM Users WHERE AppleId = ? OR Mail = ?",
          [appleToken, mail],
          function (err, tempUser) {
            if (err) {
              ...
              if (tempUser.length > 0) {
                connection.query(
                  "UPDATE Users SET AppleId = ? WHERE Id = ?",
                  [appleToken, tempUser[0].Id],
                ...
                connection.query(
                  "INSERT INTO Users(AppleId, Mail) VALUES (?, ?)",
                  [appleToken, mail],
                  function (err, dataInsert) {
                    if (err) {
                      connection.release();
  ...
  addOrUpdateGoogle(googleToken, mail, callback) {
    this.pool.getConnection(function (err, connection) {
      if (err) {
        ...
        connection.query(
          "SELECT * FROM Users WHERE GoogleId = ? OR Mail = ?",
          [googleToken, mail],
          function (err, tempUser) {
            if (err) {
              connection.release();
              ...
              if (tempUser.length > 0) {
                ...
                connection.query(
                  "INSERT INTO Users(GoogleId, Mail) VALUES (?, ?)",
                  [googleToken, mail],
                  function (err, dataInsert) {
                    if (err) {
                      connection.release();
  ...
  createApikey(userId, callback) {
    this.pool.getConnection(function (err, connection) {
      if (err) {
        ...
        let token = uuid.v4();
        connection.query(
          "INSERT INTO MobileSessions(Token, UserId) VALUES (?, ?)",
          [token, userId],
          function (err, data) {
            connection.release();
            if (err) {
  ...
  deleteApikey(apikey, callback) {
    this.pool.getConnection(function (err, connection) {
      if (err) {
        ...
        connection.query(
          "DELETE FROM MobileSessions WHERE Token = ? AND UserId = ?",
          [apikey.Token, apikey.UserId],
          function (err, data) {
            connection.release();
            if (err) {
              callback(err);
  ...
  listApiKeys(userId, callback) {
    this.pool.getConnection(function (err, connection) {
      if (err) {
        ...
        connection.query(
          "SELECT Token, CreatedAt, UsedAt, Device, UserDeviceName FROM MobileSessions WHERE UserId = ?",
          [userId],
          function (err, data) {
            connection.release();
            if (err) {
  ...
  existApikey(apikey, callback) {
    this.pool.getConnection(function (err, connection) {
      if (err) {
        ...
        connection.query(
          "SELECT * FROM MobileSessions INNER JOIN Users on UserId = Id WHERE Token = ?",
          [apikey],
          function (err, data) {
            connection.release();
            if (err) {
